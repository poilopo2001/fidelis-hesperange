...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!config) return {}
  return generatePropertyTypeMetadata('achat', config)
}

export default async function AchatPropertyTypePage({ params }: PageProps) {
  const { propertyType } = await params
  const config = getPropertyTypeBySlug(propertyType)
  if (!config) notFound()

  const properties = getPropertiesByTransactionAndPropertyType('achat', propertyType)
  const subTypes = isParentType(propertyType)
    ? getSubTypes(propertyType as 'appartements' | 'maisons')
    : undefined

  const url = `/achat/${propertyType}`
  ...
  const schemas = [
    ...
    generatePropertyTypeListingSchema('achat', config, url, properties.length),
    generateBreadcrumbSchema([

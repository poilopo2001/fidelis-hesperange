...
export default async function AchatFentangePage() {
  const properties = getPropertiesByTypeAndSection('achat', 'Fentange')
  const cityData = await getCityData(sectionId)

  const url = '/achat/fentange'
  
  const schemas = [
    generateWebPageSchema({
      url,
      title: `Acheter à ${cityData?.localite || 'Fentange'} — Fidelis Hesperange`,
    ...
    <>
      ...
      <main>
        <PropertyList
          ...
          title={cityData ? `Biens à vendre à ${cityData.localite}` : 'Biens à vendre à Fentange'}
          subtitle={cityData?.immobilier.atouts || 'Découvrez les biens en vente à Fentange'}
          headingLevel="h1"
        />
        ...
        <section className="bg-cream border-b border-stone/10">
          <div className="section-editorial py-12">
            <PropertyTypeGrid types={getParentTypes()} basePath="/achat" city="fentange" />
          </div>
        </section>
      </main>
      <Footer />

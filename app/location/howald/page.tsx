...
export default async function LocationHowaldPage() {
  const properties = getPropertiesByTypeAndSection('location', 'Howald')
  const cityData = await getCityData(sectionId)

  const url = '/location/howald'
  
  const schemas = [
    generateWebPageSchema({
      url,
      title: `Louer à ${cityData?.localite || 'Howald'} — Fidelis Hesperange`,
    ...
    <>
      ...
      <main>
        <PropertyList
          ...
          title={cityData ? `Biens à louer à ${cityData.localite}` : 'Biens à louer à Howald'}
          subtitle={cityData?.immobilier.atouts || 'Découvrez les biens en location à Howald'}
          headingLevel="h1"
        />
        ...
        <section className="bg-cream border-b border-stone/10">
          <div className="section-editorial py-12">
            <PropertyTypeGrid types={getParentTypes()} basePath="/location" city="howald" />
          </div>
        </section>
      </main>
      <Footer />
